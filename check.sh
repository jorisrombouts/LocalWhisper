...
for state in listening listening-handsfree transcribing cleaning done fallback paste-failed; do
  [ -f "$T/overlay-$state.png" ] || { echo "FAIL: overlay render: $state"; exit 1; }
done
