...
- Overlay: the `NSPanel` has a fixed generous transparent frame; the SwiftUI pill sizes itself inside it. `ProgressView` does not render in a non-activating panel; use SF Symbol effects. All icons are SF Symbols; the menu bar icon stays monochrome. Menu items use title case with an ellipsis when they open another window.
