...
- Test flags on the binary: `--transcribe file.wav`, `--record 3`, `--clean "raw text"`, `--insert-test` (pastes into the frontmost app), `--launch-at-login on|off`, `--overlay-demo <dir>` (renders every pill state to PNG).
