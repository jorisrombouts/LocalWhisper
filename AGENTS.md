...
- Whisper invents speech in non-speech audio, and its own `no_speech_prob` reads 0.00 while doing it. Silero VAD gates the decoder: only audio it hears speech in is transcribed, so hum, noise and clicks come back empty in a few ms. An amplitude threshold and a phrase deny list were both tried and both failed, the first on a faint hum, the second on every language but English. `whisper peak=` stays in the log to tell a dead microphone from one VAD heard no speech on.
