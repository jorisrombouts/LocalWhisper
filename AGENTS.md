...
- `./check.sh` is the smoke test: engine on English, Dutch and noise, cleanup, overlay render. Run it before merging. The paste path has no harness; `--insert-test` pastes into the frontmost app by hand. The hotkey and hands-free logic are tested by hand with the log's `mode=` field.
...
- Recording uses `AVCaptureSession` with an explicit `AVCaptureDeviceInput`; `AVAudioEngine` cannot bind a device reliably (Bluetooth headsets renegotiate their sample rate right after the mic opens). The converter is rebuilt when the buffer format changes. The log's `mic:` and `mic stopped:` lines show the device and the captured seconds.
- The level meter in `DictationController` tracks a noise floor and a recent peak with tuned constants; recorded silence must give zero. Change them only against the log's `peak=` values.
- Whisper hallucinates on silence ("Thank you.", "you"): clips whose loudest 100 ms stays under 0.005 RMS are skipped, segments with `no_speech_prob >= 0.6` and punctuation-only output are dropped. The log line `whisper peak=` is the calibration data.
...
- Overlay: the `NSPanel` has a fixed generous transparent frame and never activates; the SwiftUI pill sizes itself inside it and the hands-free ✕ is a tap gesture, not a button. `ProgressView` does not render in a non-activating panel; use SF Symbol effects. All icons are SF Symbols; the menu bar icon stays monochrome. Menu items use title case, with an ellipsis when they open another window. Animations ignore Reduce Motion by design.
